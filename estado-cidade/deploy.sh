...
DOCKER_IMG_NAME="motando-api-estado_cidade"
OCIR_IMG_NAME="$OCIR_ENDPOINT/$OBJSTR_NAMESPACE/$DOCKER_IMG_NAME"

function show_help() {

   cat <<EOH
MOTANDO - ESTADO CIDADE
-----------------------

-bp|--bp|--build-push       Build and Push the Docker Image to OCI Registry (OCIR).
-fp|--fp|--full-deploy      Full mode deploy (Build, Push and OKE deployment).
-h|--help                   This help.
...
function deploy() {
     ...
     echo -e "[INFO] Starting deploy API - ESTADO CIDADE ...\n"
